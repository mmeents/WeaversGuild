...
namespace Weavers.Core.Handlers.Todo {
  ...
  public class RejectTodoCommandHandler : IRequestHandler<RejectTodoCommand, RejectTodoCmdResult> {
    ...
    public RejectTodoCommandHandler(IMediator mediator, FabricDbContext context) {
      _mediator = mediator;
      _context = context;
    }

    public async Task<RejectTodoCmdResult> Handle(RejectTodoCommand request, CancellationToken cancellationToken) {
      var result = new RejectTodoCmdResult();
      ...
      if (todoCloseReasonProp != null) {
        todoCloseReasonProp.Value = request.Reason;
